...
function Dashboard() {
  // Use local JSON file for EV models
...
console.log(data);
  ...
  const dataError = !Array.isArray(data) && !loading;
  const yearOptions = useMemo(() => Array.from(new Set(safeData.map(d => d["Model Year"]))).filter(Boolean).sort().map(y => ({ value: y, label: y })), [safeData]);
  const makeOptions = useMemo(() => Array.from(new Set(safeData.map(d => d.Make))).filter(Boolean).sort().map(m => ({ value: m, label: m })), [safeData]);
  const regionOptions = useMemo(() => Array.from(new Set(safeData.map(d => d.County || d.City || d.State))).filter(Boolean).sort().map(r => ({ value: r, label: r })), [safeData]);
  ...
  }, [safeData, years, makes, regions]);

  // Compute KPI values from filtered data
  const totalEVs = filteredData.length || 0;
  const validRanges = filteredData.filter(item => item["Electric Range"] > 0);
  const avgRange = validRanges.length ? (validRanges.reduce((sum, item) => sum + item["Electric Range"], 0) / validRanges.length).toFixed(1) : "No valid range data";
  const validMsrps = filteredData.filter(item => item["Base MSRP"] > 0);
  const avgMsrp = validMsrps.length ? (validMsrps.reduce((sum, item) => sum + item["Base MSRP"], 0) / validMsrps.length).toLocaleString(undefined, { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }) : "No MSRP data";
      ...
      <Suspense fallback={<div>Loading dashboard...</div>}>
        {loading ? (
          <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
            <div className="text-lg text-gray-700 dark:text-gray-300">Loading data...</div>
          </div>
        ) : (
          <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4">
            ...
            {dataError && (
              <div className="bg-red-100 text-red-700 p-4 rounded mb-4">Error: Data failed to load or is not a valid array. Please check EVPopulationData.json format and path.</div>
            )}
            {/* Filter Description */}
            ...
            <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              ...
              <KpiCard label="Avg. Range" value={loading ? "..." : (typeof avgRange === "string" ? avgRange : `${avgRange} mi`)} />
